...
function TicketsTab() {
    ...
    const getTickets = () => {
        apiServices.get(TICKET, queryString.stringify(params))
            .then(res => {
            ...
            <div className="tickets">
                ...
                {tickets?.length && tickets?.map((item, key) => {
                    ...
                    return (
                        ...
                        <div className={"ticket-block"} data-toggle="modal" data-target="#ticket-modal" onClick={()=>{getTicketId(item?.id)}} key={key}>
                                ...
                                <div className="d-block d-md-flex box-dir-reverse">
                                    <div className="col ">
                                        <div className="d-block d-md-flex box-dir-reverse ">
                                            ...
                                            <div className="col px-0">
                                                <div className="d-flex box-dir-reverse">
                                                    <h3 className="ticket-title text-dir">
                                                        <span className={(stateTo(item?.state).type) + " ticket-status "}>{stateTo(item?.state).title}</span>
                                                    </h3>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="col">
                                        <div className="d-flex box-dir-reverse   w-100  justify-content-md-end">
                                            <span className="ticket-date  persian-num text-dir">{item?.creation_date ? moment(item?.creation_date, 'YYYY/MM/DD').locale(i18next.language === 'fa-IR' ? 'fa' : 'en').format('YYYY/MM/DD') : ""}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
