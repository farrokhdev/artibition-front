...
function ModalEditCollections(props) {
  ...
  const {
    visibleEditCollections,
    setVisibleEditCollections,
    collectionsDetails,
  } = props;
  ...
  const onFinish = (values) => {
    ...
    apiServices
      .patch(SOCIAL_NETWORK_COLLECTIONS_ID(collectionsDetails?.id), payload)
      .then((res) => {
        if (res.data) {
          message.success({
  ...
  useEffect(() => {
    if (collectionsDetails?.id) {
      ...
      setchooseProduct(
        collectionsDetails?.products?.map((details) => {
          return details?.id;
        })
      );
      form.setFieldsValue({
