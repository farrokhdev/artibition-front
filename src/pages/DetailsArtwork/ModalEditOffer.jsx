...
function ModalEditOffer(props) {
  ...
  const {
    visibleEditOfferModal,
    setVisibleEditOfferModal,
    editionValue,
    offerValue,
    productDetail,
  } = props;
    ...
    apiServices
      .patchByID(
        ...
        offerValue?.results?.map((item) =>
          item.product_item_id === editionValue?.id ? item.id : null
        ),
        payload
      )
      .then((res) => {
        if (res.data) {
          message.success({
            content: "قیمت پیشنهادی با موفقیت ثبت شد",
            style: {
              marginTop: "110px",
            },
          });
  ...
  useEffect(() => {
    setOfferValueEdit(
      offerValue?.results?.map((item) =>
        item.product_item_id === editionValue?.id ? item.toman_price : null
      )
    );
  }, [offerValue]);
        ...
        <div className="modal-content px-5">
          ...
          <div className="modal-header">
            <h5 className="modal-title" id="exampleModalLabel">
              {t("artwork.modal_edit_offer.title")}
            </h5>
          </div>
          <div className="modal-body">
            <p className="text-center">
              {t("artwork.modal_edit_offer.subtitle")}
            </p>
              ...
              <div className="col-sm-10 col-sm-offset-1">
                <div className="public-group">
                  <input
                    ...
                    defaultValue={offerValue?.results?.map((item) =>
                      item.product_item_id === editionValue?.id
                        ? item.toman_price
                        : null
                    )}
                    onChange={(e) => setOfferValueEdit(e.target.value)}
                  />
          ...
          <div className="modal-footer">
            <div className="col-sm-10 col-sm-offset-1">
              <a
                href="#"
                className="pinkcolor pull-right"
                onClick={handleClose}
              >
                {t("artwork.modal_edit_offer.cancel_offer")}
              </a>
              <button
                type="button"
                ...
                onClick={onFinish}
              >
                {t("artwork.modal_edit_offer.submit")}
              </button>
            </div>
          </div>
        </div>
