...
function ArtworksPage() {
  ...
  const filterTechniques = () => {
    ...
    return setParams(state => ({ ...state, technique_id: techniquesIdTotal}))
  }
  ...
  const getProductList = () => {
    apiServices.get(ARTIST_PRODUCTS, queryString.stringify(params, { arrayFormat: 'comma' ,skipNull: true ,skipEmptyString:true}))
      .then(res => {
        if (res.data) {
  ...
  const getProductTechniques = () => {
    apiServices.get(PRODUCTS_TECHNIQUES,queryString.stringify(techniqueParams,{arrayFormat: 'comma',skipNull:true,skipEmptyString:true}))
      .then(res => {
        if (res.data) {
  ...
  const getProductMaterials = () => {
    apiServices.get(PRODUCTS_MATERIALS,queryString.stringify(materialParams,{arrayFormat: 'comma',skipNull:true,skipEmptyString:true}))
      .then(res => {
        if (res.data) {
                ...
                <div className="panel-group" id="accordion">
                    ...
                    <div id="collapse1" className="panel-collapse collapse in">
                      <div className="panel-body">
                        <div className="nl-input filter-search">
                          <input placeholder={t("artworkList.filter.placeholder_artist")} onChange={e => setSearch(e.target.value)} />
                          <button type="button" onClick={() => setParams(state => ({ ...state, search: search }))}><img src={search_icon} width="24" height="24" alt="" />
                          </button>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="panel panel-default">
                    ...
                    <div id="collapse2" className="panel-collapse collapse in">
                      <div className="panel-body">
                          ...
                          {categories?.results?.map((item, index) =>
                            <label className="lable-checkbox text-dir">
                              <input id='category_id' name={item.id} type="checkbox" value={item.id}
                                onClick={e => {
                                  setCategoriesId({ ...categoriesId, [e.target.name]: e.target.checked })
                              }}
                              />
                              <span>
                                {i18n.language === 'fa-IR' ?
                        ...
                        <div className="d-flex justify-custom">
                          <button type="button" className="btn btn-ok pull-dir" onClick={filterCategories} >{t("artworkList.filter.size.submit")}</button>
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="panel-body">
                        ...
                        <div className="row row-inputs box-dir-reverse">
                          <div className="col-xs-6">
                            ...
                            <input type="text" className="value persian-num" data-index="0" defaultValue="0" onChange={e => i18n.language === 'fa-IR' ? setToman_price_range_min(e.target.value) : setDollar_price_range_min(e.target.value)} />
                            <span>{t("artworkList.filter.price.to")}</span>
                          </div>
                          <div className="col-xs-6">
                            <label>{t("artworkList.filter.price.max")}</label>
                            <input type="text" className="value  persian-num" data-index="1" defaultValue="0" onChange={e => i18n.language === 'fa-IR' ? setToman_price_range_max(e.target.value) : setDollar_price_range_max(e.target.value)} />
                          </div>
                        </div>
                        <div className="d-flex justify-custom">
                          <button type="button" className="btn btn-ok pull-dir"
                            onClick={() => i18n.language === 'fa-IR' ?
                              setParams(state => ({ ...state, toman_price_range_min: toman_price_range_min ? toman_price_range_min : 0, toman_price_range_max: toman_price_range_max ? toman_price_range_max : 0 }))
                              :
                              setParams(state => ({ ...state, dollar_price_range_min: dollar_price_range_min ? dollar_price_range_min : 0, dollar_price_range_max: dollar_price_range_max ? dollar_price_range_max : 0 }))}>
                  ...
                  <div className="panel panel-default">
                    ...
                    <div id="collapse7" className="panel-collapse collapse in">
                      <div className="panel-body">
                        <div className="nl-input filter-search ">
                          <input placeholder={t("artworkList.filter.technique.placeholder")} onChange={e => setTechniqueSearch(e.target.value)}/>
                          <button className="noborder" type="button" onClick={() => setTechniquesetParams(state => ({ ...state, search: techniqueSearch }))}><img src={search_icon} width="24" height="24" alt="" />
                          </button>
                          <div className="md-mrgt30">
                          ...
                          <div className="d-flex justify-custom">
                          <button type="button" className="btn btn-ok pull-dir" onClick={filterTechniques} >{t("artworkList.filter.size.submit")}</button>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="panel panel-default">
                    ...
                    <div id="collapse9" className="panel-collapse collapse in">
                      <div className="panel-body ">
                        <div className="nl-input filter-search ">
                          <input placeholder={t("artworkList.filter.material.placeholder")} onChange={e => setMaterialSearch(e.target.value)}/>
                          <button className="noborder" type="button" onClick={() => setMaterialParams(state => ({ ...state, search: materialSearch }))}><img src={search_icon} width="24" height="24" alt="" />
                          </button>
                          ...
                          <div className="d-flex justify-custom">
                          <button type="button" className="btn btn-ok pull-dir" onClick={filterMaterials} >{t("artworkList.filter.size.submit")}</button>
                        </div>
                      </div>
                    </div>
                  </div>
                        ...
                        <div className=" row-inputs togglevisibility">
                          <div className="row mrgt16 box-dir-reverse">
                            <div className="col-xs-6 text-dir">
                              ...
                              <input type="number" className="value persian-num" onChange={e => setWidth_min(e.target.value) } />
                              <span >{t("artworkList.filter.size.in")}</span>
                            </div>
                            <div className="col-xs-6 text-dir">
                              <label>{t("artworkList.filter.size.max_width")}</label>
                              <input type="number" className="value  persian-num" onChange={e => setWidth_max(e.target.value) } />
                            </div>
                          </div>
                          <div className="row mrgt16 box-dir-reverse">
                            <div className="col-xs-6 text-dir">
                              <label>{t("artworkList.filter.size.min_length")}</label>
                              <input type="number" className="value persian-num" onChange={e => setLength_min(e.target.value)} />
                              <span>{t("artworkList.filter.size.in")}</span>
                            </div>
                            <div className="col-xs-6 text-dir">
                              <label>{t("artworkList.filter.size.max_length")}</label>
                              <input type="number" className="value  persian-num" onChange={e => setLength_max(e.target.value)} />
                            </div>
                          </div>
                          <div className="row mrgt16 box-dir-reverse">
                            <div className="col-xs-6 text-dir">
                              <label>{t("artworkList.filter.size.min_height")}</label>
                              <input type="number" className="value persian-num" onChange={e => setHeight_min(e.target.value)} />
                              <span>{t("artworkList.filter.size.in")}</span>
                            </div>
                            <div className="col-xs-6 text-dir">
                              <label>{t("artworkList.filter.size.max_height")}</label>
                              <input type="number" className="value  persian-num" onChange={e => setHeight_max(e.target.value)} />
                            </div>
                          </div>
                        </div>
                        <div className="d-flex justify-custom">
                          <button type="button" className="btn btn-ok pull-dir" onClick={() => width_min || width_max || height_min || height_max || length_max || length_min ? setParams(state => ({ ...state, width_min: width_min,width_max:width_max,height_min:height_min,height_max:height_max,length_min:length_min,length_max:length_max })) : filterSizes()} >{t("artworkList.filter.size.submit")}</button>
                        </div>
                      </div>
                    </div>
                  </div>
